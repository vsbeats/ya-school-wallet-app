...
class FileModel extends Model {
	...
	async getById(id) {
		const entity = this._dataSource.find((i) => i.id === id);
		if (!entity) throw new AppError(`Entity with id ${id} not found`, 404);
	...
	async remove(id) {
		...
		console.log('entity from model', entity);
		const entityIndex = this._dataSource.find((i) => i.id === id);
		if (!entityIndex) throw new AppError('Can\'t find entity index in database file', 400);
